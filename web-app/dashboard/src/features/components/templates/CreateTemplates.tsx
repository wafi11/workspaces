...
const onDragVRight = useCallback((e: MouseEvent) => {
  ...
  return (
   <main className="flex flex-col w-full h-screen">
        <TopbarAdmin title='Create Templates' className='mb-0'>
              <button
                type="button"
                onClick={handleSubmit}
                className="p-2 md:px-4 md:py-2 rounded-md text-xs sm:text-sm "
                style={{ background: 'var(--color-sidebar-text-active)', color: 'var(--color-sidebar-bg)', cursor: 'pointer', border: 'none' }}
              >
                Create Template
              </button>
        </TopbarAdmin>
            <div ref={rootRef} className="flex w-full min-h-screen overflow-y-auto">
                {/* Left panel */}
                <div ref={leftRef} style={{ width: '50%' }} className="overflow-hidden flex flex-col">
                   <div ref={topLeftRef} style={{ height: '50%' }} className="overflow-hidden flex flex-col">
                                            <FormTemplates form={form} />

                    </div>
                     <div
                        onMouseDown={initDrag(onDragVLeft)}  // ← onDragV bukan onDragH
                        style={{ height: 2, background: '#202020', cursor: 'row-resize', flexShrink: 0 }}
                    />
                ...
                <div
                    ...
                    style={{ width: 2, background: '#202020', cursor: 'col-resize', flexShrink: 0 }}
                    />

                {/* Right panels */}
                <div ref={rightRef} className="flex-1  flex flex-col overflow-hidden">
                    {/* Top right */}
                    <div ref={topRightRef} style={{ height: '50%' }} className="overflow-hidden flex flex-col">
