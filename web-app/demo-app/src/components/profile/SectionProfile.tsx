...
import { ButtonUploadPhoto, DialogUploadPhoto } from "../ui/button-upload-photo";
import { CopyableValue } from "../ui/copy-valueablle";
import { GetInitials } from "../ui/get-initials";
import { useState } from "react";
import { useUpdatePhotoProfile } from "@/features/api/user/profile";

interface ProfileSectionProps {
    user: User;
}
export function ProfileSection({ user }: ProfileSectionProps){
  const [photoClick,setPhotoClick]  = useState<boolean>(false)
  const {mutate}  = useUpdatePhotoProfile()
    return (
      <>
      ...
      <div className="bg-[#0a0a0a] border border-[#1c1c1c] rounded-xl overflow-hidden">
        ...
        <div className="relative flex items-center gap-4 px-6 py-5 border-b border-[#161616]">
          ...
          <div className="relative flex-shrink-0">
            ...
            <ButtonUploadPhoto onClick={() => setPhotoClick(true)} />
          </div>
          ...
          <div className="flex-1 min-w-0">
            ...
            >
              <span className={`w-1 h-1 rounded-full ${user.role === "admin" ? "bg-[#a78bfa]" : "bg-[#3f3f46]"}`} />
      ...
      {
        photoClick && (
          <DialogUploadPhoto onClose={() => setPhotoClick(false)} onSubmit={(data) => mutate({avatar_base64 : data})} open={photoClick}/>
        )
      }
      </>
